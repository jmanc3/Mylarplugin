...
PKG_FLAGS := $(shell pkg-config --cflags librsvg-2.0 libdrm hyprland pangocairo wayland-server  wayland-client wayland-cursor xkbcommon cairo dbus-1 alsa)
PKG_LIBS  := $(shell pkg-config --libs   librsvg-2.0 libdrm hyprland pangocairo wayland-server wayland-client wayland-cursor xkbcommon cairo dbus-1 alsa)
