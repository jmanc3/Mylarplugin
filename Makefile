...
PKG_FLAGS := $(shell pkg-config --cflags librsvg-2.0 libdrm hyprland pangocairo wayland-server  wayland-client wayland-cursor xkbcommon cairo gtk+-3.0 dbus-1 alsa libpipewire-0.3 libpulse gio-2.0)
PKG_LIBS  := $(shell pkg-config --libs librsvg-2.0 libdrm hyprland pangocairo wayland-server wayland-client wayland-cursor xkbcommon cairo gtk+-3.0 dbus-1 alsa libpipewire-0.3 libpulse gio-2.0)
...
load: all unload
	sleep 1s; hyprctl plugin load ${PWD}/$(OUTPUT)
