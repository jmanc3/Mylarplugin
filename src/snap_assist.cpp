        ...
        for (auto o : add) {
            ...
            thumb->when_paint = [](Container *actual_root, Container *c) {
                ...
                if (parent_data->should_slide) {
                    fadea = ((float) (get_current_time_in_ms() - parent_data->slide_start)) / (800.0f * ratioscalar);
                    if (fadea > 1.0)
                        fadea = 1.0;
                    fadea = pull(slidetopos2, fadea);
                }
