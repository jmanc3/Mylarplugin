...
-- Default springs
hl.curve("easy",           { type = "spring", mass = 1, stiffness = 578.5, dampening = 67.9 })
...
--    plugin = {
--        mylardesktop = {
--            sel_border_color = "rgba(5fffe5aa)",
--            sel_color = "rgba(5fffe544)",
--        }
--    }
--})
...
std::string conf_path() {
    #ifdef NDEBUG
        return ".config/mylar/default.lua";
    #else
        return ".config/mylar/debug_default.lua";
    #endif
}

inline CFunctionHook* g_pOnGetMainConfigPathHook = nullptr;
typedef std::expected<Config::Supplementary::Jeremy::SConfigStateReply, std::string> (*origGetMainConfigPath)();
std::expected<Config::Supplementary::Jeremy::SConfigStateReply, std::string> hook_onGetMainConfigPath() {
#ifdef TRACY_ENABLE
    ZoneScoped;
#endif
#ifdef FORK_WARN
...
Bounds bounds_popup(int id) {
    ...
    for (auto hp : hyprpopus) {
        if (hp->id == id) {
            if (auto p = hp->p) {
                if (p->m_mapped && !(p->m_windowOwner.expired() && p->m_layerOwner.expired())) {
...
class CExpoGesture : public ITrackpadGesture {
  public:
    ...
    void begin(const ITrackpadGesture::STrackpadGestureBegin& e) {
        ...
        if (start_func)
            start_func({e.swipe->delta.x, e.swipe->delta.y, 0, 0});
